  ...
  <div class="app-container">
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column label="供货商" width="100" align="center">
          <template slot-scope="scope">{{ scope.row.wareHouse }}</template>
        </el-table-column>
        <el-table-column label="销量" width="100" align="center">
          <template slot-scope="scope">{{ scope.row.sellNum }}</template>
        </el-table-column>
  ...
  methods: {
    ...
    getBrandName() {
      for (let i = 0; i < this.list.length; i++) {
        for (let j = 0; j < this.brandOptions.length; j++) {
          if (this.list[i].brandId == this.brandOptions[j].value) {
            // this.list[i].brandName = this.brandOptions[j].label;

            this.$set(this.list[i], "brandName", this.brandOptions[j].label);
          }
        }
      }
    },
    ...
    getCategoryName() {
      for (let i = 0; i < this.list.length; i++) {
        let oneCategoryStr = "";
        let twoCategoryStr = "";
        let threeCategoryStr = "";
        let oneCategoryIds, twoCategoryIds, threeCategoryIds;
        for (let j = 0; j < this.productCateOptions.length; j++) {
          if (this.list[i].oneCategoryId == this.productCateOptions[j].value) {
            oneCategoryStr = this.productCateOptions[j].label;
            oneCategoryIds = this.productCateOptions[j].value;
          }
          if (this.list[i].twoCategoryId == this.productCateOptions[j].value) {
            twoCategoryStr = this.productCateOptions[j].label;
            twoCategoryIds = this.productCateOptions[j].value;
          }
          if (
            this.list[i].threeCategoryId == this.productCateOptions[j].value
          ) {
            threeCategoryStr = this.productCateOptions[j].label;
            threeCategoryIds = this.productCateOptions[j].value;
          }
        }
        let outStr = oneCategoryStr;
        if (oneCategoryIds != twoCategoryIds) {
          outStr = outStr + "/" + twoCategoryStr;
        }
        if (twoCategoryIds != threeCategoryIds) {
          outStr = outStr + "/" + threeCategoryStr;
        }
        // console.log(
        //   outStr + oneCategoryStr + twoCategoryStr + threeCategoryStr
        // );
        // this.list[i].category = outStr;
        this.$set(this.list[i], "category", outStr);
      }
    },
    handleSearchEditSku() {
    ...
    updatePublishStatus(publishStatus, ids) {
      ...
      updatePublishStatus(ids).then((response) => {
        this.$message({
          message: "修改成功",
          type: "success",
          duration: 1000,
    ...
    updateVerifyStatus(VerifyStatus, ids) {
      ...
      updateVerifyStatus(ids).then((response) => {
        this.$message({
          message: "修改成功",
          type: "success",
          duration: 1000,
